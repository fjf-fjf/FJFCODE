...
#define MMAP_SIZE   800*480*4 //映射内存大小

#define GIF_NUM     72        //开机动画帧数

#define  TS_DEV   "/dev/input/event0" //触摸屏设备文件路径
...
int main()
{
   ...
   //打开图片并且申请映射，显示图片出来
   Bmp_Decode("/tmp/3.bmp",lcd_mp); //显示出来相册和录像集

   //点击屏幕任意位置进入主页面
   get_touch_xy(&touch_x,&touch_y,ts_fd);
    if((touch_x>142)&&(touch_x<300)&&(touch_y>151)&&(touch_y<308))
             {
     ...
     if((touch_x>507)&&(touch_x<663)&&(touch_y>151)&&(touch_y<308))
             {
     ...
     if((touch_x>724)&&(touch_x<800)&&(touch_y>0)&&(touch_y<80))
             {
